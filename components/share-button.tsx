...
export function ShareButton({ url, title, description, showLabel = false, className }: ShareButtonProps) {
  ...
  return (
    <>
      <button
        onClick={handleShare}
        className={className || "flex items-center gap-1 text-sm text-gray-600 dark:text-gray-400 hover:text-primary"}
      >
      ...
      {showModal && (
        ...
        >
          <div
            className="bg-white dark:bg-gray-900 rounded-t-2xl sm:rounded-2xl w-full max-w-md p-5 animate-in slide-in-from-bottom sm:slide-in-from-bottom-0 duration-300"
            ...
            <div className="grid grid-cols-4 gap-3 mb-5">
              {shareLinks.map((link) => (
                <a
                  ...
                  onClick={() => setShowModal(false)}
                >
                  <div className={`w-12 h-12 rounded-full ${link.color} text-white flex items-center justify-center`}>
                    {link.icon}
                  </div>
